...
class GuestbookadminController extends AdminbaseController{
	...
	function sdkaccess()
	{
		$count=$this->sdkaccess_model->where(array("status"=>1))->count();
		$page = $this->page($count, 20);
		$guestmsgs=$this->sdkaccess_model->where(array("status"=>1))->order(array("createtime"=>"DESC"))->limit($page->firstRow . ',' . $page->listRows)->select();
		$this->assign("Page", $page->show('Admin'));
		$this->assign("guestmsgs",$guestmsgs);
		$this->display();
	}

	function sdkaccessdelete(){
		$id=intval(I("get.id"));
		$result=$this->sdkaccess_model->where(array("id"=>$id))->delete();
		if($result!==false){
			$this->success("删除成功！", U("Guestbookadmin/sdkaccess"));
		}else{
			$this->error('删除失败！');
		}
	}
